...
class EmberApp {
  ...
  constructor(defaults, options) {
    ...
    this._activeAddonInclude = [];
    this._importOwners = new Map();
  ...
  */
  _notifyAddonIncluded() {
    ...
    this.project.addons.forEach(addon => {
      if (addon.included) {
        this._trackAddonMethod('included', addon, () => addon.included(this));
      }
    });
  }

  _trackAddonMethod(methodName, addon, fn) {
    if (methodName !== 'included') {
      return fn();
    }
    try {
      this._activeAddonInclude.push(addon);
      return fn();
    } finally {
      this._activeAddonInclude.pop();
    }
  }
  ...
  */
  populateLegacyFiles() {
    let owner = this.project.findAddonByName('ember-source');
    this._trackAddonMethod('included', owner, () => {
      let name;
  ...
  */
  ...
  _import(assetPath, options, directory, subdirectory, extension) {
    let owner = this._activeAddonInclude[this._activeAddonInclude.length - 1] || this;
    if (!this._importOwners.has(owner)) {
      this._importOwners.set(owner, { imports: [] });
    }
    this._importOwners.get(owner).imports.push(assetPath);
  ...
  prebuildV1Addons() {
    let VanillaPackages = require('ember-cli-vanilla').default;
    let vanillaPackages = new VanillaPackages(this);
    return mergeTrees(vanillaPackages.dumpTrees());
  }
}
