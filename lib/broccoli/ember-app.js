...
class EmberApp {
  ...
  */
  ...
  _import(assetPath, options, directory, subdirectory, extension) {
    ...
    if (!owner._trackedImports) {
      owner._trackedImports = [];
    }
    owner._trackedImports.push({ assetPath, options });
  ...
  toVanilla() {
    let { VanillaApp, V1InstanceCache } = require('ember-cli-vanilla');
    let cache = new V1InstanceCache(this);
    let vanillaApp = new VanillaApp(this.project.root, cache);
    return mergeTrees(vanillaApp.dumpTrees());
  }
}
