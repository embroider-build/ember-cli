...
class EmberApp {
  ...
  */
  ...
  prebuildV1Addons() {
    // when discovering v1 addons to prebuild as v2 packages, we only need to
    // traverse the active ones. Non-active packages v2 packages can only
    // provide ownJavascript and CSS, but v1 packages did not offer those
    // features, so we can always ignore them.
    //
    // Our builder function necessarily recurses depth-first, because each v2
    // package needs to have its deps in v2 form too in order to be really ready
    // to go.
    let { prebuildV1Package } = require('ember-cli-vanilla');
    return mergeTrees(this.project.addons.map(prebuildV1Package));
  }
}
